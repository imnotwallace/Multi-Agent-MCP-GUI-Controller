...
10. **Create sqlite-vec table** [✅]

Using the sqlite-vec extension, create a new Virtual Table named "context_chunk_embeddings" (using underscores for consistency with other table names) to store vector embeddings for each chunk. The table should be created via a migration script. The schema for the virtual table should be as follows:
...
Ensure that the vector embeddings are indexed for efficient similarity search. Use the appropriate indexing method provided by sqlite-vec for 256-dimensional vectors. Ensure that the index is created when the server starts up if it does not already exist.
...
## TODO: Individual System/Script Updates Required

Based on the tasks above, the following systems and scripts need to be updated to implement the placeholder functionality:

### 1. **redesigned_mcp_server.py** - Database Schema & Core Logic ✅
- ✅ Implement new `context-chunks` table schema with foreign key relationship to `contexts` table
- ✅ Create `context_chunk_embeddings` virtual table using sqlite-vec extension
- ✅ Add database indexing for `context_id` and `created_at` columns in `context-chunks` table
- ✅ Update WriteDB method to implement chunking logic using RecursiveCharacterTextSplitter (3500 char chunks with 15% overlap)
- ✅ Update ReadDB method to retrieve last 10 chunks with optional `since` timestamp parameter and permission filtering
- ✅ Implement VectoriseChunks method with nomic.ai embedding model integration (256-dimensional vectors)
- ✅ Add nomic model initialization on server startup
- ✅ Implement asynchronous vectorization process that runs after WriteDB completion

### 2. **redesigned_comprehensive_gui_hub.py** - Main GUI Controller ✅
- ✅ Update permission level references from "full" to "session" throughout the interface (already implemented correctly)
- ✅ Ensure proper imports and calls to modular GUI components
- ✅ Verify correct file references and subprocess calls

### 3. **gui_modules/contexts_tab.py** - Contexts Tab UI Updates ✅
- ✅ Rename "Context" column to "Context Summary" displaying first 100 characters of first chunk
- ✅ Add "No. of Chunks" column showing chunk count per context
- ✅ Implement "View Chunks" button functionality
- ✅ Create new chunks viewer window with sequential chunk display
- ✅ Add "Add Chunk" functionality with 4025 character UI limit
- ✅ Update context data retrieval to work with new chunk-based schema

### 4. **gui_modules/projects_tab.py** - Projects & Sessions UI Layout ✅
- ✅ Reorganize bottom buttons into two rows:
  - ✅ Row 1: + New Project, + New Session, Rename
  - ✅ Row 2: Refresh, Delete

### 5. **Database Migration Script** - Schema Updates ✅
- ✅ Create migration script to add `context-chunks` table
- ✅ Create `context_chunk_embeddings` virtual table with sqlite-vec
- ✅ Add necessary indexes for performance optimization
- ✅ Handle data migration from existing `contexts` table structure

### 6. **requirements.txt** - Dependencies ✅
- ✅ Add langchain dependency for RecursiveCharacterTextSplitter
- ✅ Add nomic dependency for embedding model
- ✅ Add sqlite-vec extension dependency (sqlite-vec already present)
- ✅ Add numpy dependency for vector operations (already present)

### 7. **Text Processing Module** - Chunking Logic ✅
- ✅ Implement `vectorise_text_chunks()` function with nomic embedding model
- ✅ Create chunk splitting logic ensuring sentences aren't split across chunks
- ✅ Implement 3500 character limit with 15% overlap functionality
- ✅ Add error handling for chunking operations

### 8. **Permission System Updates** - Throughout Codebase ✅
- ✅ Update all references to permission levels: self, team, session, project
- ✅ Replace "full" permission references with "session"
- ✅ Add "project" permission level handling
- ✅ Update permission validation logic in all relevant modules
